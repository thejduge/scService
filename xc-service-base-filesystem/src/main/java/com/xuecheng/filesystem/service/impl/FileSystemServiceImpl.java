...
public class FileSystemServiceImpl implements FileSystemService {
    ...
    public UploadFileResult upload(MultipartFile file, String businesskey, String filetag, String metadata) {
        ...
        //1.将文件上传到fastDFS中,返回一个文件id
        String fileId = this.fdfs_upload(file);
        System.out.println("文件保存到dfs系统成功,返回的文件id为:"+fileId);
        if (StringUtils.isEmpty(fileId)){
            //上传文件失败
            ExceptionCast.cast(FileSystemCode.FS_UPLOADFILE_SERVERFAIL);
        }
        //2. 将文件id和其他的文件信息存储到mongodb中
     ...
     * @return 存储文件后生成的fileID
     */
    private String fdfs_upload(MultipartFile multipartFile){
        //初始化fastDFS的环境
        this.initFastDFS();
        //获得trackerClient,请求trackerServer
        TrackerClient trackerClient = new TrackerClient();
        try {
            //连接tracker
            TrackerServer trackerServer = trackerClient.getConnection();
            //获得stroageClient服务器,创建stroageClient
    ...
    private void initFastDFS(){
        try {
            ...
            e.printStackTrace();
            //初始化失败抛出异常不在继续执行
            ExceptionCast.cast(FileSystemCode.FS_ENVIRONMENT_INITFAIL);
        }
    }
}
